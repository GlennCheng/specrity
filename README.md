...
Speckit-Jira 為 PM 和工程師提供 4 個核心 AI Agent 指令，串接 Jira 與 Spec-Kit 的 PRD 管理流程：

| 指令 | 角色 | 說明 |
|------|------|------|
| `/pm.specify` | PM | 從 Jira Ticket 起草 PRD → Clarify → 建 Branch → 更新 Jira |
| `/dev.plan` | Engineer | 讀 PRD → 分析程式碼 → 產出 `plan.md` |
| `/dev.tasks` | Engineer | 讀 plan + spec → 拆解 tasks → 建 Jira Sub-tasks |
| `/dev.implement` | Engineer | 逐步實作 → 自動勾進度 → 更新 Jira |
...
Speckit-Jira 採用 **「個人安裝」** 模式：workflow 檔案只存在使用者本機，不 commit 到專案 repo。
...
Lead 只需要設定**專案層級的共用設定**（`.speckit-jira.yml` + spec submodule），不需要部署 workflow。
...
### Step 2: 建立 `.speckit-jira.yml`

在專案根目錄建立設定檔：

```yaml
# .speckit-jira.yml — commit 到 repo
spec_mode: submodule
spec_path: specs/
```
...
```bash
git add .speckit-jira.yml
git commit -m "chore: add speckit-jira project config"
git push
```

> **只有 `.speckit-jira.yml` 和 submodule 會 commit 到 repo。Workflow 檔案不 commit。**

---

## 個人安裝（每個想用的人做一次）

### Step 1: Clone speckit-jira

```bash
# 建議放在固定位置，之後更新也方便
git clone https://github.com/your-org/speckit-jira.git ~/speckit-jira
```
...
# 安裝到你使用的 AI 工具
~/speckit-jira/install.sh agy            # Antigravity
~/speckit-jira/install.sh cursor-agent   # Cursor IDE
~/speckit-jira/install.sh claude         # Claude Code CLI
~/speckit-jira/install.sh --help         # 查看所有支援的工具
```

`install.sh` 會：
1. 偵測專案根目錄
2. 讀取 `.speckit-jira.yml`（如果 Lead 已設定）或互動式建立
...
```
/pm.specify HTGO2-123     ← 開始起草 PRD
/dev.plan HTGO2-123       ← 產出實作計畫
/dev.tasks HTGO2-123      ← 拆解任務 + 建 Jira Sub-tasks
/dev.implement HTGO2-123  ← 逐步實作
```

### ⚙️ Workflow 更新

speckit-jira 的 workflow 版本更新時：

```bash
cd ~/speckit-jira && git pull         # 更新 speckit-jira
cd ~/my-project
~/speckit-jira/install.sh agy         # 重新部署到你的工具
```
...
```bash
# Antigravity
rm -rf ~/.agents/workflows/pm.specify.md  # etc.

# Cursor
rm .cursor/rules/speckit-jira.mdc

# Claude Code
# 手動移除 CLAUDE.md 中的 speckit-jira 區塊
```
...
```
/pm.specify HTGO2-123  → drafts/HTGO2-123/ 寫入
/pm.specify HTGO2-456  → drafts/HTGO2-456/ 寫入（不影響 123）
/pm.specify HTGO2-123  → 偵測 drafts/HTGO2-123/ 存在 → Resume
...
新對話 `/pm.specify HTGO2-123` → 讀取這些檔案 → 從上次中斷處繼續。
...
中途切換任務？
  /pm.specify HTGO2-456  ← 開始另一個
  /pm.specify HTGO2-123  ← 回來繼續
```

### 🔧 Engineer 工作流

```
/dev.plan HTGO2-123       → 讀 PRD → 分析程式碼 → 產出 plan.md
/dev.tasks HTGO2-123      → 拆解任務 → 產出 tasks.md → 建 Jira Sub-tasks
/dev.implement HTGO2-123  → 逐步實作 → 更新進度 → 更新 Jira 狀態
```
...
| `cursor-agent` | Cursor IDE | `.cursor/rules/speckit-jira.mdc` |
| `windsurf` | Windsurf | `.windsurf/rules/speckit-jira.md` |
| `agy` | Antigravity | `.agents/workflows/*.md` |
| `qwen` | Qwen Code | `QWEN.md` |
| `opencode` | opencode | `AGENTS.md` |
| `codex` | Codex CLI | `AGENTS.md` |
| `kilocode` | Kilo Code | `.kilocode/rules/speckit-jira.md` |
| `auggie` | Auggie CLI | `.augment/rules/speckit-jira.md` |
| `roo` | Roo Code | `.roo/rules/speckit-jira.md` |

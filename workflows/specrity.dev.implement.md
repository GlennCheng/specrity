...
> 確認 Jira Ticket 已到達適合開始實作的狀態。

1. 使用 MCP `getJiraIssue` 取得 Ticket 當前狀態
2. 讀取 `jira_transitions` 中的 `implement_start` 設定
3. 若 Ticket 當前狀態早於預期：
   ```
   ⚠️ 此 Ticket 目前狀態為「{current_status}」，Tasks 可能尚未通過確認。
   建議確認任務拆解已完成後再開始實作。
   (1) 仍然繼續（我確認過了）
   (2) 中止
   ```
...
6. 讀取 PRD（**Jira Ticket → Confluence → 本地**）— 取得需求背景：先從 Jira Ticket 掃描欄位、description、comments 尋找 Confluence PRD 連結；若找到則使用 MCP `getConfluencePage`（`contentFormat = "markdown"`）拉取；若找不到則檢查本地 `state.yml` 或 `drafts/<TICKET_ID>/prd.md`

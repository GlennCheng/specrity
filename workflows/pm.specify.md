...
3. **Checkpoint（每輪結束後）**：AI 對照 spec-template 評估覆蓋率，用一句話告訴 PM：
   - 若仍有重要面向未覆蓋：`「還有 N 個面向不太清楚（XX、XX），要繼續嗎？」`
   - 若覆蓋率足夠：`「主要面向都釐清了，剩下的我用合理預設填寫。要繼續還是進入定稿？」`
   - PM 說「繼續」→ 進入下一輪
   - PM 說「可以了」/「定稿」/「done」→ 進入 Step 3

...
> **整合原生 Specrity 的 `tasks` + `taskstoissues` 兩個指令。**
...
> 確認 Jira Ticket 已到達適合開始 Tasks 拆解的狀態。

1. 使用 MCP `getJiraIssue` 取得 Ticket 當前狀態
2. 讀取 `jira_transitions` 中的 `tasks_start` 設定
3. 若 Ticket 當前狀態早於預期（例如 Plan 尚未完成 Review）：
   ```
   ⚠️ 此 Ticket 目前狀態為「{current_status}」，Plan 可能尚未通過 Review。
   建議確認 Tech Lead 已在 Confluence 審核 Plan 後再開始 Tasks 拆解。
   (1) 仍然繼續（我確認過了）
   (2) 中止
   ```
4. 此檢查為「軟擋」，使用者選擇繼續則照常執行

#### 0-E: Confluence Review Comment 檢查

> 發布前檢查 Plan 的 Confluence 頁面是否有未解決的 Review Comment。

1. 若 `plans/<TICKET_ID>/state.yml` 有 `confluence_plan_page_id`，使用 MCP `getConfluencePageInlineComments`（`resolutionStatus = "open"`）
2. 若有未解決 comment → 展示警告並讓使用者選擇繼續或中止
3. 若無 → 通過

4. 讀取 `plan.md` 和 PRD
   - 讀取 `$SPEC_ROOT/plans/<TICKET_ID>/plan.md`
   - 讀取 PRD（**Jira Ticket → Confluence → 本地**）：先從 Jira Ticket（已在 Step 0-B 取得的 ticket 資訊中）掃描欄位、description、comments 尋找 Confluence PRD 連結；若找到則使用 MCP `getConfluencePage`（`contentFormat = "markdown"`）拉取 PRD；若找不到則檢查本地 `drafts/<TICKET_ID>/state.yml` 的 `confluence_page_id`；最後才讀取本地 `drafts/<TICKET_ID>/prd.md`
...
### Step 5: 發布 Tasks 到 Confluence

> 將 tasks.md 發布到 Confluence，作為 PRD 頁面的子頁面。

1. **取得 PRD 的 Confluence page ID**：
   - 從 Jira Ticket 或本地 state.yml 取得 `confluence_prd_page_id`
   - 若找不到，使用 `$CONFLUENCE_PARENT_PAGE_ID` 作為父頁面

2. **建立或更新 Confluence 頁面**：
   - 檢查 `plans/<TICKET_ID>/state.yml` 是否已有 `confluence_tasks_page_id`
   - **若無（首次發布）** → 使用 MCP `createConfluencePage`：
     ...
     參數:
       ...
       title = "[<TICKET_ID>] <Ticket Title> — Tasks"
       body = tasks.md 完整內容
...
3. **記錄到 `plans/<TICKET_ID>/state.yml`**：
   ```yaml
   confluence_tasks_page_id: <page_id>
   confluence_tasks_url: <page_url>
   ```

4. **更新 Jira Ticket**：將 Tasks 的 Confluence 連結加到 Ticket comment

5. **若 Confluence 連線失敗**：不阻擋流程，tasks.md 已在本地完成。
...
- **Confluence 頁面**：`[TICKET_ID] Title — Tasks`（掛在 PRD 頁面底下）
- Jira Sub-tasks（若 MCP 可用）
- Jira comment 摘要 + Confluence 連結
